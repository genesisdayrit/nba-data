...
def create_missing_crawlers(bucket_name, folder_prefix, database_name, role_arn):
    ...
    for page in page_iterator:
        for prefix in page.get('CommonPrefixes', []):
            ...
            if f'{folder_name}-update-crawler' not in existing_crawler_names and f'{folder_name}-update-crawler' not in existing_crawler_names:
                # Create a new crawler for the folder
                new_crawler = {
                    'Name': f'{folder_name}-update-crawler',
                    'Role': role_arn, 
